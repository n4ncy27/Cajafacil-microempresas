...
class _GastosPageState extends State<GastosPage> {
  ...
  Future<void> _registrarPorVoz(BuildContext context) async {
    final texto = await VoiceListeningModal.show(
      context,
      titulo: 'Dicta tu gasto',
      ejemplo: '"Pagué 150000 de arriendo"',
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          resultado.isValid
              ? 'Gasto detectado: ${resultado.descripcion} por ${_formatearPesos(resultado.monto!)}'
              : 'Texto reconocido: "$texto" — Módulo de gastos próximamente',
          style: GoogleFonts.montserrat(fontSize: 13),
        ),
        backgroundColor: resultado.isValid ? Colors.green : Colors.red.shade300,
  ...
  Future<void> _abrirNuevoGasto() async {
    final ok = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => _NuevoGastoPage(onVoz: _registrarPorVoz),
      ),
    );
...
class _NuevoGastoPage extends StatefulWidget {
  /// Callback para abrir el asistente de voz (recibe el BuildContext del caller)
  final Future<void> Function(BuildContext) onVoz;

  const _NuevoGastoPage({required this.onVoz});
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () => widget.onVoz(context),
